-- Decalre the global variables for parts 1 and 2
DECLARE entity_id_var STRING;
DECLARE test_start_date, test_end_date DATE;
DECLARE asa_ids ARRAY <INT64>;
SET entity_id_var = "FP_PH";
SET (test_start_date, test_end_date) = (DATE("2022-10-01"), DATE("2022-10-31"));
SET asa_ids = [487, 573];

-- Part A: ASA Versioning
-- Step 1: Create a table with the versions of a particular ASA sorted from the newest to the oldest
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_overview_asa_and_scheme_versioning` AS
...
SELECT
  ...
  *,
FROM versioning_tbl
WHERE DATE(active_from) BETWEEN test_start_date AND test_end_date -- This is the part where we only filter for the ASA versions that existed during the lifetime of the experiment
...
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_change_log_asa_and_scheme_versioning` AS
WITH filter_asas AS (
  ...
  INNER JOIN `dh-logistics-product-ops.pricing.asa_overview_asa_and_scheme_versioning` ver ON fcv.entity_id = ver.entity_id AND fcv.asa_id = ver.asa_id AND fcv.active_from = ver.active_from
...
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_flags_non_component_related_changes_details_asa_and_scheme_versioning` AS
WITH apply_lag_func AS (
  ...
  FROM `dh-logistics-product-ops.pricing.asa_change_log_asa_and_scheme_versioning`
)
...
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_flags_non_component_related_changes_asa_and_scheme_versioning` AS
SELECT
  entity_id,
  asa_id,
...
FROM `dh-logistics-product-ops.pricing.asa_flags_non_component_related_changes_details_asa_and_scheme_versioning`
GROUP BY 1,2;

-- Part 2: Component related changes
-- Step 3.3: ASA flags component related changes details
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_flags_component_related_changes_details_asa_and_scheme_versioning` AS
...
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_flags_component_related_changes_asa_and_scheme_versioning` AS
SELECT
  entity_id,
  asa_id,
...
FROM `dh-logistics-product-ops.pricing.asa_flags_component_related_changes_details_asa_and_scheme_versioning`
GROUP BY 1,2;
...
CREATE OR REPLACE TABLE `dh-logistics-product-ops.pricing.asa_flags_asa_and_scheme_versioning` AS
SELECT *
FROM `dh-logistics-product-ops.pricing.asa_flags_non_component_related_changes_asa_and_scheme_versioning` a
LEFT JOIN `dh-logistics-product-ops.pricing.asa_flags_component_related_changes_asa_and_scheme_versioning` b USING(entity_id, asa_id)
